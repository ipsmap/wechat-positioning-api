...
* **URL**
 ...
  https://map.ipsmap.com/#/map?id=MPA_ID&partner=PARTNER_ID&poi=POI_LINK&cburl=CALLBACK_URL&arrmsg=ARR_MSG
...
*  **URL Params**
   ...
   **Required:**
   ...
   `POI_LINK` 兴趣点 poi id，需与服务方共同依循的poi id，若POI对应有问题，请直接与现场工程师联系 
 
   `CALLBACK_URL` 回调网址，当符合条件时，道一循服务器会访问回调网址（类似webhook作用方式），请留意此url需经过编码（可使用encodeURIComponent），回调网址需经过我方认证，详细请参考下方Notes。
